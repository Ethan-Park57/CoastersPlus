...
remove_outliers <- function(df) {
  ...
  df_filtered <- df %>%
    mutate(across(where(is.numeric), ~replace(., . < quantile(., 0.25) - 1.5 * IQR(.), NA))) %>%
    mutate(across(where(is.numeric), ~replace(., . > quantile(., 0.75) + 1.5 * IQR(.), NA))) %>%
    drop_na()
...
for (pitch_type in pitch_types) {
  ...
  df[, numeric_cols] <- remove_outliers(df[, numeric_cols])
