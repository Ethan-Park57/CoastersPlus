...
```{r League Movement & Slider Plots, warning = FALSE, message = FALSE, echo = FALSE}
results <- pitches %>% 
...
```{r Movement Plots, echo = FALSE, warning = FALSE, message = FALSE}
pitches <- pitches %>%
  mutate(Type = 
           case_when(
...
```{r Pitch Prop Axis Inverted, echo = FALSE, warning = FALSE, message = FALSE}
Data2 %>% 
...
#### Sliders
```{r Slider Tree & Random Forest, echo = FALSE}
...
```{r Fastball Tree & Random Forest, echo = FALSE}
...
#### Curveballs
```{r Curveball Tree & Random Forest, echo = FALSE}
...
```{r Change-Up Tree & Random Forest, echo = FALSE}
...
test_caret$results$RMSE
```

```{r Classification Table FINAL, echo = FALSE, eval = FALSE}
Stf_Class <- c("< 55", "< 70", "< 85", "< 95", " < 105", 
              "< 115", "< 130", "< 145", "< 160", "≥ 160") %>% 
  cbind(1:10) %>% 
  as.data.frame()
...
```{r GAM CV, echo = FALSE, message = FALSE}
groups <- Data2 %>%
  select(pitcher_id) %>% 
  distinct(pitcher_id) %>% 
  rowwise() %>%
...
cv_m <- gam(est_woba ~ s(avg_speed) + s(spin_rate) + s(pitcher_break_x) +
            s(pitcher_break_z), data = cv_d, family = gaussian, method = "REML")
...
```{r Model 2 GAM / RF CV, echo = FALSE, message = FALSE}
# RMSE Function
rmse <- function(observed, predicted){
  n <- length(observed)
  sqrt(sum((observed - predicted)^2) / n)}
