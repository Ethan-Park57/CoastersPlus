...
##### Change-Ups
```{r Chang-Ups RV, warning = FALSE, message = FALSE, echo = FALSE}
grv_ch_1 <- Data2 %>% 
  filter(pitch_type == "CH" | pitch_type == "FS") %>% 
  ggplot(aes(x = spin_rate, y = rv100, 
             color = `Stf+ Pitch`)) +
...
#### wOBA {.tabset}
##### Sliders
```{r Sliders 2 wOBA, warning = FALSE, message = FALSE, echo = FALSE}
g2_sl_1 <- Data2 %>% 
  filter(pitch_type == "SL") %>% 
...
grid.arrange(g2_sl_1, g2_sl_2, g2_sl_3, g2_sl_4, ncol = 2, top = "wOBA on Sliders by Spin Rate & Speed")
```
...
# g2_sl_3 <- Data2 %>% 
#   filter(pitch_type == "SL") %>% 
#   ggplot(aes(x = pitcher_break_x, y = pitcher_break_z, 
#              color = `Stf+ Pitch`)) +
#   geom_point(alpha = 0.7, show.legend = FALSE) +
#   geom_smooth(method = lm, se = FALSE, show.legend = FALSE) + 
#   labs(x = "Vertical Movement (in.)",
#        y = "Horizontal Movement (in.)",
#        subtitle = "Pitch Movement Plot") +
#   scale_color_gradient(low = "red", high = "green") +
#   theme_dark()+
#   theme(plot.subtitle = element_text(hjust = 0.5),
#         axis.text.y = element_blank(),
...
# g2_sl_4 <- Data2 %>% 
#   filter(pitch_type == "SL") %>% 
#   ggplot(aes(x = speed_diff, y = woba, 
#              color = `Stf+ Pitch`)) +
#   geom_point(alpha = 0.7) +
#   geom_smooth(method = lm, se = FALSE) + 
#   labs(x = "Speed Differential (mph)",
#        y = "wOBA",
#        color = "Stuff+",
#        subtitle = "Speed Differential (vs. Primary Fastball)") +
#   scale_color_gradient(low = "red", high = "green") +
#   theme_dark()+
#   theme(plot.subtitle = element_text(hjust = 0.5),
#         axis.text.y = element_blank(),
